...
public class DataStreamSerializer implements StreamSerializer {
    ...
    try (DataOutputStream dos = new DataOutputStream(os)) {
      ...
      writeCollection(dos, r.getSections().entrySet(), entry -> {
        ...
        switch (type) {
          case PERSONAL:
          case OBJECTIVE:
            dos.writeUTF(((SectionLine) section).getContent());
            break;
          case ACHIEVEMENT:
          case QUALIFICATION:
            writeCollection(dos, ((SectionList) section).getContent(), dos::writeUTF);
            break;
          case EXPERIENCE:
          case EDUCATION:
            writeCollection(dos, ((SectionOrganization) section).getOrganizations(), org -> {
  ...
  private Section readSection(DataInputStream dis, SectionType sectionType) throws IOException {
    switch (sectionType) {
      case PERSONAL:
      case OBJECTIVE:
        return new SectionLine(dis.readUTF());
      case ACHIEVEMENT:
      case QUALIFICATION:
        return new SectionList(readList(dis, dis::readUTF));
      case EXPERIENCE:
      case EDUCATION:
        return new SectionOrganization(
